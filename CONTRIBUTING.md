...
See the [`CI GH Action`](.github/workflows/ci-verify-build.yml) on how to run the tests suites.
...
Make sure the [`Dev Journey Action`](.github/workflows/ci-verify-dev-journey.yml) is passing. You may run it locally using the `./scripts/dev-journey-*.sh` scripts. See the full workflow in the [`CI GH Action`](.github/workflows/ci-verify-dev-journey.yml).
